...
#   MSE-forecasts, see tutorial 0. For white noise the MSE-forecast is the mean i.e. 0.
...
# SSA emphasizes zero-crossings or sign-changes. 
...
# The empirical holding-time is larger for the two-sided target
...
# Set-up a matrix with the competing filters
...
# Plot shifted series: this seems about right...
ts.plot(short_sample_shifted,col=c("black","green"),lty=1:2,main="Shift predictor (green line) by 3 units to the left")
abline(h=0)

# We can infer the lagor right-shift directly from the transfer function of the filter or, more precisely, 
...
#   Therefore the error is the cummulated sum of future xt with variance
sum(sigma^2*gamma[1:((L-1)/2+delta)]^2)

# The MSE converges to the expected `true' MSE for longer time series

# We can now think of the above numbers in terms of forecast performances
# A `good' predictor should have a small MSE, a small lag and nearly the same ht as the target 
...
for (i in 1:length(delta_vec))# i<-1
{
  ...
  print(i)
  print(compute_holding_time_func(b_mse)$ht)
  # MSE  
  if (((L-1)/2+delta)>0)
  {  
  ...
  {
    # Backcast horizon is such that the target can be replicated perfectly (no future data involved)
    mse<-c(mse,0)
  }
...
#   -Note that the lag here always refers to a nowcast
#   -the effective lag relative to the target should be corrected by delta_vec[i])
...
#   2. MSE-performances are adequat
# Trilemma (Part I): 
...
# Final notes
# 1. SSA currently addresses timeliness by changing delta
...
# -However, their trilemma does not optimize MSE conditional on ht and shift
#   -Besides Timeliness and Smoothness, the third constituent of the AST-trilemma, called Accuracy, 
#     measures properties of the amplitude function of the filter in the passband
# -SSA improves on the AST-trilemma mainly in terms of interpretability
#   -the third constituent, namely MSE, is a familiar concept   