# In this tutorial we explain the SSA criterion in example 1, based on a simple example
...
# There exist alternative approaches for predicting signs of target
#   -We argue that SSA is more efficient than these!
#   -In particular, we benchmark SSA to a classic logit model in example 2
...
# The SSA criterion emphasizes zero-crossings or sign accuracy: it wants to maximize the probability that the 
#   predictor yt forecasts the sign of the target z_{t+delta}

# We first compute the empirical sign-accuracy
sum((sign(y_mse*y_sym)+1)/2,na.rm=T)/length(na.exclude(y_sym))
# If xt and thus yt and zt are gaussian, then proposition 1 links empirical and expected or true sign-accuracy
...
SA_true
# We can also compute the empirical estimate
...
# Discussion:
# -By relying on the closed-form `true' expression SA_true for SA, we can link the predictor to SA and maximize accuracy as 
...
#   -Maximize rho_yz under a constraint for ht
...
# Note: the correlation with the target, SA, ht or the lag-one ACF are indifferent to affine transformations
#   -The solution of SSA is determined up to an arbitrary scaling constant s: any s is allowed 
#   -We solve this undeterminacy by computing that particular scaling s which maximizes MSE-performances, see section
#     2 in JBCY paper
...
# Note that the scale of the logit-model predictor is `arbitrary': but we here consider SA only
sum(b_mse^2)
sum(b_logit^2)
