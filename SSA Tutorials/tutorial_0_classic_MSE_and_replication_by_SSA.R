...
#   as a soubassement for engrafting SSA onto (in view of modifying its characteristics).
...
#   -This is the same as obtaining forecasts of y_one_sided
# -An estimate of y_sym at the sample end t=len is called a nowcast: in the above example, we have to compute 
#   forecasts for the missing future data x_{len+1}, x_{len+2},x_{len+3} missing in the symmetric filter. 
...
#   Try by increasing len the length of the time series...
...
# For this we first specify the number of equidistant frequency ordinates in [0,pi] (we omit negative frequencies which are a miiror-image of the positive ones)
K<-600
...
# The shift in the passband of the filter is roughly one time unit, which confirmes our previous findings
...
#   -No other competing design can outperform y_mse in terms of MSE performances (systematically, on the long run)
...
# Note that this filter is not symmetric anmyore!
...
# Increasing the length len of the data would lead to arbitrarily tight fits of empirical and expected ht
...
# As an effect, the lag (phase-shift in passband) is slightly smaller, though still nearing one
...
# This examples illustrates the case of `splitted' filtering addressed in section 2 of the JBCY paper:
#   -We consider a generally acausal filter Gamma such that zt=Gamma(xt) 
#   -We consider also a causal (ARMA-) filter Xi such that xt=Xi(epsilont)
# The first filter Gamma is often interpreted as signal extraction filter: how can we `extract' interesting components 
#   from the data xt (for example a trend, a cycle or a seasonally adjusted component) 
# The traditional view on the second filter Xi is that it corresponds to a model of the data. But we can try
#   to abstract from the model-perspective and conceive Xi as a filter, whose input is white noise epsilont with output 
#   xt. By looking at the data, instead of epsilont, we push the first filter Xi aside. But the latter cannot be 
#   ignored anymore when deriving relevant estimates and performance measures.
# We can then merge both filters into a single combined or convolved filter Gamma(Xi(xt))
#   -The latter can be used to infer the MSE-estimate, the expected holding-time, the spectral density of the predictor 
#     and the true or expected MSE
# Therefore, our SSA function computes two optimal filters: the one applied to xt, called ssa_x, and the convolved 
#     or merged filter, applied to epsilont, called ssa_eps (we omitted the eps reference in its naming)
#   -The output of both filters is the same: an optimal predictor of the target or signal 
#   -But ssa_x is applied to the original data and is typically used for computing the predictor
#   -ssa_eps is typically used for deriving true or expected performances: ht, MSE, correlations as well as the 
#     spectral density of the predictor
...
#   ht of the MSE-predictor
# Let's challenge this claim here
...
# Nowcast i.e. delta=0
delta<-0
# Recall that the (symmetric) target gamma is right-sifted by (length(gamma)-1)/2=3 time units: it is a one-sided causal filter
...
# SSA also computes the MSE-filters: as applied to xt and epsilont
...
#   We just have to correct for the different lengths of MSE and two-sided target, see proposition 4 in JBCY paper
...
# First criterion
cor(na.exclude(cbind(y_mse,y_ssa)))
# Second criterion
cor(na.exclude(cbind(y_sym,y_ssa)))
...
#   We could improve tightness of the approximation by increasing the number of iterations (of the numerical optimizations)
...
#   -Proposition 4 in JBCY paper shows that both targets are equivalent
