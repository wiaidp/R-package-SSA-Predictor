...
# We briefly illustrate HF with the R-package neverhpfilter (never use HP...)
# Data is retrieved from neverhpfilter package
data(GDPC1)
...
# We skip pandemic: outliers affect HF-regression
...
# Stated differently: the forecast and the future observation are not properly cointegrated
...
#  the unit-root; otherwise the residuals wouldn't be stationary
# The fact that the sum of the Hamilton filter coefficients doesn't vanish is a slight 
#   drawback (it is due to overfitting) when compared to HP (the gap filter coefficients sum to zero: but HP-gap should not be used for BCA, see tutorials 2 and 5)
sum(hamilton_filter)
# We now correct the filter such that the sum of the coefficients is zero (we just distribute the difference evenly across coefficients)
# Alternative corrections could be envisioned but the rounding-error (to achieve a zero-sum) is much smaller than the 
#   regression sampling error and therefore we can ignore such subtleties here (you are welcome to experiment)
...
# Compute corresponding adjusted residuals: forecast residuals are cycle (note that we do not adjust for the intercept anymore)
...
# For the purpose of engrafting SSA onto Hamilton-filter, we here consider the adjusted and un-centered cycle 
...
# Next step: in order to plug SSA on HF we have to transform the empirical setting such that the data is stationary, see section 2.3 in JBCY paper
#   -Recall that the concept of zero-crossings does not make sense for non-stationary (integrated) series 
# Proceeding: we transform the Hamilton filter such that it can be applied to first differences (instead of levels)
...
# -We can always find a new filter, called ham_diff below, which, when applied to differences (instead of levels), replicates the output of the original bandpass, as applied to levels
...
#   -The vanishing coefficients are obtained through our above transformation of HF
...
par(mfrow=c(1,1))
ts.plot(residual_diff,col="blue",main="Outputs of filters applied to levels and to differences: both series overlap")
...
# In principle we can ignore this mismatch because a translation of the series does not affect SSA-computation
...
#-----------------------------------------------------------------------
# 1.4 Autocorrelation
# In the case of autocorrelation, section 2 in JBCY paper suggests a method for extending the SSA-framework
...
#   -This assumption is used in section 4 of the JBCY paper, when applying HP and SSA to INDPRO, see tutorial 5
...
# But our main purpose is to derive a smoother (SSA-) design: smoother relative to Hamilton filter  
# Therefore, we can augment ht of the Hamilton filter by 50% 
# As a result, the SSA filter output will be smoother
...
# It matches our target: proof that the optimization converged to the global optimum (this is invariably the case in applications because the optimization of 'non exotic' forecast problems is fairly easy)
ht_obj$ht 
ht

#--------------------------------------------------
# 1.5 Filter series and compute performance measures
# Our version of the Hamilton filter does not remove the changing drift or slope: this can be desirable 
...
# Apply Hamilton to data and compare holding-times
...
# 2. We must account for different (arbitrary) scalings of SSA_out, cycle_diff and original_hamilton_cycle
...
# 1. The Hamilton filter ham_diff (applied to returns) is a lowpass (the original Hamilton filter, applied to levels, is a bandpass)
...
#     -The Hamilton filter does not generate spurious cycles; but neither does HP-trend applied to returns (see tutorial 5)
#     -In contrast, HP-gap and BK generate spurious cycles: filter-artifacts
# 2. The amplitude functions of both SSA are closer to zero (smaller than Hamilton filter) towards higher frequencies 
#     -Effect: stronger smoothing and less (noisy) zero-crossings
#     -This typical property of SSA-smoothing designs has coined the term 'noisy crossings' (of the target filter)
#     -Typically, SSA damps undesirable high-frequency components (noise) more effectively than the target onto which it has been engrafted
...
#     -But HP-trend applied to returns performs well and its lag is smaller, see tutorial 2.

########################################################################################################################
#################################################################################################################
# Example 2
# Application of SSA to Hamilton filter based on monthly PAYEMS series (non-farm payroll) 
...
# Example 4 emphasizes post-1990 data only (great moderation)

# Complete example where all features of the current (simple univariate) SSA are illustrated
# We make some simplifying assumptions along the analysis in order to emphasize the relevant facets of SSA 
#   -we discard the singular pandemic
#   -We assume the differenced data to be white noise or to follow an ARMA (both are misspecified, at least for the long sample, starting pre WWII)
#   -Outcome: the relative performance gain of SSA is not affected by our simplifying assumptions

# Let's briefly illustrate the Hamilton filter as applied to monthly PAYEMS: R-package neverhpfilter
library(neverhpfilter)
data(GDPC1)
...
ts.plot((log(PAYEMS)))
# Returns are non stationary: slowly drifting and series pre/post 1960 as well as pre/post 1990
#   differ  in term of dependence structure and variance (WWII, `great moderation') 
# We here ignore these issues: our main purpose is to illustrate SSA; moreover SSA is more or less insensitive to the misspecification
...
ts.plot(cbind(residuals,lm_obj$residuals),main="Replication of HF (both series overlap)")

# The coefficients nearly sum-up to one: this is because the series is trending and therefore the filter must remove 
#  the unit-root; otherwise the residuals wouldn't be stationary
sum(hamilton_filter)
# We now correct the filter such that the sum of the coefficients is zero (we just distribute the difference evenly on coefficients)
hamilton_filter_adjusted<-hamilton_filter
hamilton_filter_adjusted[(h+1):(h+p)]<-hamilton_filter_adjusted[(h+1):(h+p)]-sum(hamilton_filter)/p
# Now the sum is zero
sum(hamilton_filter_adjusted)
...
#---------------------------------------------------
# 2.2 Transformation: from levels to differenced
# Next step: in order to plug SSA on HF we have to transform the empirical setting such that the data 
...
ham_diff<-conv_with_unitroot_func(hamilton_filter_adjusted_L)$conv
par(mfrow=c(2,1))
# Note that the coefficients of the filter vanish for lags larger than length of hamilton_filter_adjusted and therefore we could set L=length(hamilton_filter_adjusted)
ts.plot(ham_diff,main="Hamilton filter as applied to first differences")
ts.plot(hamilton_filter_adjusted_L,main="Hamilton filter as applied to level")
...
# The main tool for checking autocorrelation is the ACF:
acf(x,main="ACF")
# The ACF suggests dependency among the returns of GDPC1
...
# Although the series look similar, the Hamilton filter has more (noise-) leakage (see amplitude functions below) 
...
# We can clearly see the non-stationarity of the cycles, too
compute_empirical_ht_func(SSA_out)
compute_empirical_ht_func(ham_out)
...
#   shorter time-frames (for example discard data prior 1990): see example 4 below
compute_empirical_ht_func(scale_shifted_SSA)
compute_empirical_ht_func(original_hamilton_cycle)
...
# The scale of the forecast is smaller because the estimation problem is more difficult (zero-shrinkage)
...
# The nowcast SSA is synchronized with Hamilton filter; the forecasts are left-shifted (leading) and smoother
max_lead=8
# SSA nowcast is synchronized with Hamilton
shift_obj<-compute_min_tau_func(mplot[,c(1,4)],max_lead)
# SSA half-year forecast is leading Hamilton by ~3 months 
shift_obj<-compute_min_tau_func(mplot[,c(2,4)],max_lead)
# SSA full-year forecast is leading Hamilton by ~6 months
...
#  the relative phase-shift differences in the passband closely
...
########################################################################################################################
#################################################################################################################
# Example 3: same as example 2 but we assume the data to be autocorrelated
...
ht_ham_conv_obj$ht
# Compare with ht of ham_diff
ht_ham_diff_obj$ht
# Compare with empirical ht: residuals_adjusted is the output of ham_diff
compute_empirical_ht_func(residuals_adjusted)
# We see that part of the ht bias of ham_diff has been resolved by ham_conv by modelling the ACF-structure of the data
...
# Check holding time constraint: 
#   Both numbers are identical (up to rounding) confirming convergence of the optimization to the global maximum
...
# Although the series look quite similar, the Hamilton filter has more (noise-) leakage and therefore it generates 
...
# The forecast SSA filter has a lead at zero-crossings relative to Hamilton filter (and less crossings)
max_lead=8
# SSA nowcast is synchronized with Hamilton
shift_obj<-compute_min_tau_func(mplot[,c(1,4)],max_lead)
...
# This will affect 
#   1. The definition of the Hamilton cycle because the regression parameters will change (quite substantially, indeed)
...
# Strongly different regression parameters (as compared to example 3 above)!!!
...
# Both cycles (forecast residual) differ in terms of `levels' , see discussions in previous examples
par(mfrow=c(2,2))
ts.plot(y,main="Log(PAYEMS)")
ts.plot(cbind(residuals,residuals_adjusted),col=c("red","blue"),main="Cycles")
mtext("Classic cycle",col="red",line=-1)
...
#-------------------------------------------------------------------------------
# 4.3 Holding-times
# We first compute the holding-time of the new (newly fitted) Hamilton-filter ham_diff
ht_ham_diff_obj<-compute_holding_time_func(ham_diff)
# Much shorter holding time than in examples 2 (and 3)
ht_ham_diff_obj$ht 
ht_ham_example2
...
# Check holding time constraint
ht_obj<-compute_holding_time_func(SSA_filt_ham_diff_eps)
# Matching numbers confirm that the optimization has converged to global maximum
ht_obj$ht 
ht
...
# Empirical holding times: SSA approximately 50% larger than Hamilton (deviation commensurate with sampling error)
compute_empirical_ht_func(SSA_out)
compute_empirical_ht_func(ham_out)
...
# Discussion: see the discussion at the end of example 1 above  
# Findings
# -Removing the remote past (from WWII up to 1990), the shortened time series allows to fit a better model to the 
...
#   -deviations are compatible with random sampling error
# The positive phase-shift or lag of HF is smaller than in the previous examples
#   -The HF-filter depends more or less strongly on the sample chosen for estimating parameters of the regression equation
...
#     -Don't use HP-gap, applied to the original data: yes, see confirmation in tutorial 2.
#     -But HP-trend applied to returns performs well and its lag is smaller, see tutorial 2.
#     -And HP-trend does not depend on the data-window or sample-size (this argument is two-sided)


#----------------------------------------------------------------------------------------------
# 4.9 To conclude we apply the above filters `till the end of the data sample, including the pandemic
# Instructive outcome!
...
#   -a longer forecast horizon leads to a faster zero-decay of filter coefficients
...
#   -In contrast, the nowcast (or the Hamilton target) would need another 10-11 months to `forget' the singularity
# It is not clear at this point "why you should never use the HP"
#   -The impulse response of the classic HP-trend decays faster and it is smoother than HF
#   -Therefore the effects of gross outliers (pandemic) are less pronounced
...
#     -White noise and ARMA generated fairly similar results on the long time span 
#     -Empirical holding times are generally strongly biased. But correcting for misspecifications reduced this bias (see results for adjusted SSA vs. original Hamilton cycle)
